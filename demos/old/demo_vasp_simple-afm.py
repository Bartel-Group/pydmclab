...
def get_magmoms(
    query, max_afm_combos=20, savename="magmoms_%s.json" % FILE_TAG, remake=False
):
...
def check_magmoms(query, magmoms):
    for mpid in magmoms:
...
def main():
    ...
    magmoms = get_magmoms(query=query, remake=remake_magmoms)

    if print_magmoms_check:
        check_magmoms(query=query, magmoms=magmoms)
