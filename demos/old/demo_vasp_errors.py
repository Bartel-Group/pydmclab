...
if USE_INSTALLED_PYDMC:
    ...
    from pydmc.handy import (
        read_json,
        write_json,
        read_yaml,
        write_yaml,
        is_slurm_job_in_queue,
    )
...
def prepare_calc_and_launch(
    ...
    user_configs["partition"] = (
        "msismall"  # choose a partition (change this if jobs aren't starting quickly)
    )
    user_configs["nodes"] = 1  # choose # nodes (increase if jobs are taking too long)
    user_configs["ntasks"] = (
        8  # choose # tasks (increase if jobs are taking too long; decrease if trouble getting jobs running in a "small" queue
    )

    sub = SubmitTools(launch_dir=launch_dir, magmom=magmom, user_configs=user_configs)
    sub.write_sub
    if ready_to_launch:
        os.chdir(launch_dir)
...
def launch_calcs(
    ready_to_launch=False, fresh_restart=False, refresh_configs_yaml=False
):
    """
    ...
    for MPID in MPIDS:
        ...
        for MAG in MAGS:
            ...
            if MAG == "nm":
                fake_error(
                    MPID, MAG, "gga-relax", "internal error in subroutine IBZKPT"
                )
...
def main():
    ...
    if running_on_msi:
        launch_calcs(
            ready_to_launch=False, fresh_restart=True, refresh_configs_yaml=False
        )

        launch_calcs(
            ready_to_launch=True, fresh_restart=False, refresh_configs_yaml=False
        )
