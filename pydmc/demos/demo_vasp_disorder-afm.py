...
Let's say we want to consider various Ir concentrations (x = 0, 1/8, 1/4, 1/2, 3/4, 7/8, 1)
...
def get_strucs(query,
    ...
    for x in x_values:
        ...
        data[str(int(8*x))] = strucs
    return write_json(data, fjson)

def check_strucs(strucs):
    for x in strucs:
        print('\n')
        print('x = %.4f' % float(int(x)/8))
...
If we want to run VASP on N configurations, it's probably a good idea to set 
    ...
    - for this example, let's say our goal is 4 AFM configurations per structure
...
def get_launch_dirs(strucs,
    ...
    for x in strucs:
        for i in strucs[x]:
            ...
            structure = strucs[x][i]['structure']
            curr_magmoms = magmoms[x][i]
            top_level = x
            ID = i
...
def main():
    ...
    """
    launch_configs = {'n_afm_configs' : 4,
                      'standards' : ['dmc'],
                      'final_xcs' : ['metagga']}
