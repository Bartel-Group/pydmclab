...
from pydmclab.hpc.helpers import set_magmoms_from_template
...
def main():
    ...
    # reference bulks are made in the slabs launcher
    # see pydmclab.hpc.helpers.get_slabs()
    ...
    rjson = os.path.join(DATA_DIR, "strucs.json")
    if not os.path.exists(rjson):
        raise FileNotFoundError(
            f"File {rjson} not found. Check that reference bulks have been generated and copied over."
        )
    strucs = read_json(rjson)
