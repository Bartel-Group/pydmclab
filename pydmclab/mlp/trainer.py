...
class OPTIMIZERS(TorchOptimizerEnum):
    """An enumeration of optimizers to update model weights"""

    sgd = (torch.optim.SGD, {"momentum": 0.9, "weight_decay": 0})
    adam = (torch.optim.Adam, {"weight_decay": 0})
    adamw = (torch.optim.AdamW, {"weight_decay": 1e-2})
    radam = (torch.optim.RAdam, {"weight_decay": 0})


class SCHEDULERS(TorchLrSchedulerEnum):
    """An enumeration of learning rate schedulers to update learning rates"""

    multisteplr = (
        torch.optim.lr_scheduler.MultiStepLR,
        {
            "milestones": lambda context, kwargs: [
                4 * context.get("epochs", 50),
                6 * context.get("epochs", 50),
                8 * context.get("epochs", 50),
                9 * context.get("epochs", 50),
            ],
            "gamma": 0.3,
        },
    )
    exponentiallr = (torch.optim.lr_scheduler.ExponentialLR, {"gamma": 0.98})
    cosineannealinglr = (
        torch.optim.lr_scheduler.CosineAnnealingLR,
        {
            "T_max": lambda context, kwargs: 10 * context.get("epochs", 50),
            "eta_min": lambda context, kwargs: kwargs.pop("decay_fraction", 1e-2)
            * context.get("learning_rate", 1e-3),
        },
    )
...
class CHGNetTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        stress_loss_ratio: float = 0.1,
        mag_loss_ratio: float = 0.1,
        optimizer: TorchOptimizer | str = "Adam",
        scheduler: TorchLrScheduler | str = "CosineAnnealingLR",
        criterion: str = "MSE",
        epochs: int = 50,
        ...
        self.trainer_args = (
            self._context | optimizer_kwargs | scheduler_kwargs | criterion_kwargs
        )
        self.model = model
        self.targets = targets

        self.optimizer: TorchOptimizer = (
            OPTIMIZERS[optimizer.lower()](
                self.model.parameters(), learning_rate, **optimizer_kwargs
            )
            if isinstance(optimizer, str)
            else optimizer
        )

        self.scheduler: TorchLrScheduler = (
            SCHEDULERS[scheduler.lower()](
                self.optimizer, self._context, **scheduler_kwargs
            )
    ...
    def _init_keys(self) -> list[str]:
        return [
            key
            for key in list(inspect.signature(CHGNetTrainer.__init__).parameters)
