...
class DiffusivityAnalyzer(MSONable):
    def __init__(
        ...
        spec_dict = (
            ...
            else {"lower_bound": 4.5, "upper_bound": 0.5, "minimum_msd_diff": 4.5}
        )
        if not {"lower_bound", "upper_bound", "minimum_msd_diff"} <= set(
            spec_dict.keys()
        ):
        ...
        # time intervals, dt
        dt_indices = np.arange(
            1, n_steps, max(int((n_steps - 1) / time_intervals_number), 1)
        )
        ...
        def one_ion_msd_fft(r, dt_indices):
            ...
            for i in range(n_step):
                r_square_sum = (
                    r_square_sum - r_square[i - 1, :] - r_square[n_step - i, :]
                )
        ...
        for i in range(n_ions):
            msd_i, msd_component_i = one_ion_msd_fft(
                displacements_final_diffusion_ions[i, :, :], dt_indices
            )
            msd_by_ions = np.append(msd_by_ions, msd_i.reshape(1, n_dt), axis=0)
            msd_component_by_ions = np.append(
                msd_component_by_ions, msd_component_i.reshape(3, 1, n_dt), axis=1
            )
        ...
        if (
            ...
            or msd[upper_bound_index] - msd[lower_bound_index]
            < spec_dict["minimum_msd_diff"]
        ):
            slope = -1
            intercept = -1
            ...
            slope, intercept, _, _, _ = stats.linregress(
                dt[lower_bound_index : upper_bound_index + 1],
        ...
        self.drift = drift
        self.drift_maximum = np.max(np.abs(drift), axis=1)[
            0
        ]  # the maximum drift vector of the framework ions, shape is (3,)
    ...
    @classmethod
    def from_vaspruns(
        cls, vaspruns, specie, time_intervals_number=1000, spec_dict=None
    ):
        """
...
class ErrorAnalysisFromDiffusivityAnalyzer(object):
    def __init__(self, diffusivity_analyzer, site_distance=3.0):
        ...
        """
        n_jump = (
            len(diffusivity_analyzer.indices)
            * np.max(diffusivity_analyzer.msd)
            / (site_distance * site_distance)
        )
        n_jump_component = (
            len(diffusivity_analyzer.indices)
            * np.max(diffusivity_analyzer.msd_component, axis=1)
            / (site_distance * site_distance)
        )
    ...
    def get_summary_dict(self, oxidized_specie=None):
        ...
        d["diffusivity_component_relative_standard_deviation"] = (
            self.RSD_D_component * d["diffusivity_components"]
        )
        return d
...
class ArrheniusAnalyzer(object):
    def __init__(self, temperatures, diffusivities, diffusivity_errors=None):
        ...
        if diffusivity_errors is None:
            ...
            y_error = [
                np.log10(np.e) * diffusivity_errors[i] / diffusivities[i]
                for i in range(len(diffusivities))
            ]
            [slope, intercept], cov = curve_fit(
                linear, x, y, sigma=y_error, absolute_sigma=True
            )
            slope_sigma = np.sqrt(np.diag(cov))[0]
            intercept_sigma = np.sqrt(np.diag(cov))[1]
    ...
    def predict_diffusivity(self, temperature):
        ...
        logD_sigma = np.sqrt(
            np.power(self.slope_sigma * (1000.0 / temperature), 2)
    ...
    def from_csv(cls, csv_file):
        ...
        if set(data_keys) >= set(["T", "D", "D_error"]):
            ...
            raise Exception(
                "Please make sure the header name in csv file is T, D, D_error(optional)"
            )
...
def get_conversion_factor(structure, specie, temperature):
    ...
    if n == 0:
        raise Exception(
            "No specie {} in the structure composition: {}".format(
    ...
    e = 1.6021766208e-19
    R = 8.3144598
    return 1000 * n / (vol * N_A) * z**2 * (N_A * e) ** 2 / (R * temperature)
