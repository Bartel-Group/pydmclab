...
def mus_from_mp_no_corrections_matbench_source():
    """
    From Matbench Discovery (https://github.com/janosh/matbench-discovery/blob/main/matbench_discovery/data-files.yml#L14)
    """

    fjson = os.path.join(DATA_PATH, "mus_from_mp_no_corrections_matbench_source.json")
    if os.path.exists(fjson):
        return read_json(fjson)

    sjson = os.path.join(DATA_PATH, "2023-02-07-mp-elemental-reference-entries.json")
    ...
    for el in els:
        mus_mp_matbench[el] = sjson[el]["energy"] / sjson[el]["composition"][el]

    mus_mp_matbench = dict(sorted(mus_mp_matbench.items()))

    return write_json(mus_mp_matbench, fjson)


def mus_from_omat():
    """
    From https://huggingface.co/datasets/facebook/OMAT24/tree/main/references
    """
